...
class Handler:
    ...
    async def sendLongPres(self, message, output, is_delete=None):
        await message.reply(output) if len(output) <= 4000 else await message.reply_document(document=BytesIO(output.encode(), name="result.txt"))
        if is_delete: await is_delete.delete()
